...
class CommentRow extends Component {
  ...
  deleteComment = () =>{
    ...
    if (window.confirm("정말 삭제하시겠습니까?")) {
      axios
        .delete(`http://54.180.89.24:8080/comment/delete/${this.props.comment._id}`, send_param)
        //정상 수행
        .then((response) => {
  ...
  updateComment = () =>{
    ...
    if(window.confirm('정말 수정하시겠습니까?')){
      ...
      axios
        .put(`http://54.180.89.24:8080/comment/update/${this.props.comment._id}`, send_param)
        .then(response => {
          if(response.data.message){
            this.setState({
    ...
    return (
      ...
      <Comment style={{marginBottom:20}}>
        <Comment.Avatar 
          ...
          style={imgStyle} /> 
        <Comment.Content>
          <Comment.Author onClick={this.handleUserProfileShow}  as='a'>{this.props.user.name}</Comment.Author>
        ...
        <>
          ...
          <Comment.Text>{this.state.content}</Comment.Text> 
            ...
            {this.props.comment.user_id === Number($.cookie('user_unique_id'))|| $.cookie('user_type') === 'SUPER'
	     ?
              <>
...
class CommentIndex extends Component {
  ...
  getCommentIndex = () => {
    ...
    axios
      .get(`http://54.180.89.24:8080/comment/index/post_id=${send_param.post_id}`)
      .then(response => {
        let comments;
