...
class PwChangeModalForm extends Component {
  ...
  pwChange = () => {
    ...
    {
      alert("새로운 비밀번호를 숫자와 문자, 특수문자 포함 8~16자리로 입력해주세요.");
      this.newPw.value = "";
      this.confirmNewPw = "";
      this.newPw.focus();
      return;
    ...
    if (confirmNewPw !== newPw){
      alert("새로운 비밀번호가 새로운 비밀번호 확인과 다릅니다. 새로운 비밀번호 확인을 다시 입력해주세요!");
    ...
    axios
      .put(`http://54.180.89.24:8080/user/${$.cookie('user_unique_id')}/changePw`, send_param)
      //정상 수행
      .then(response => {
